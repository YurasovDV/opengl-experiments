...
namespace SimpleShooter.Graphics
{
    public class ShaderLoader
    {
        ...
        private static ShaderProgramDescriptor LoadTextureLess()
        {
        ...
        private static ShaderProgramDescriptor LoadTextureLessNoLight()
        {
            ShaderProgramDescriptor result = new ShaderProgramDescriptor();
            var textureLessProgId = GL.CreateProgram();

            var vert = GL.CreateShader(ShaderType.VertexShader);
            var vertText = File.ReadAllText(@"Content\Shaders\texturelessNoLight.vert");
            GL.ShaderSource(vert, vertText);
            GL.CompileShader(vert);
            GL.AttachShader(textureLessProgId, vert);
            ...
            var frag = GL.CreateShader(ShaderType.FragmentShader);
            var fragText = File.ReadAllText(@"Content\Shaders\texturelessNoLight.frag");
            GL.ShaderSource(frag, fragText);
            GL.CompileShader(frag);
            GL.AttachShader(textureLessProgId, frag);
